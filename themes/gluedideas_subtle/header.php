...
<div id="container">
	...
	<div id="header">
		<div class="style_content">
			...
			<h1 id="title"><a href="<?php echo get_option('home'); ?>/"><span><?php bloginfo('name'); ?></span></a></h1>
			<p id="tagline"><span><?php bloginfo('description'); ?></span></p>
			
			<ul id="menu">
				<li class="<?php echo($sMenuHome); ?>"><a href="<?php echo get_option('home'); ?>/"><?php _e('Home', 'gluedideas_subtle'); ?></a></li>
				<?php wp_list_pages('exclude=68&depth=1&title_li=0&sort_column=menu_order'); ?>
			</ul>
            
                        <?php // added subpage navigation for pages 
            ...
            if (!$children && $post->post_parent) {
              $cpost = get_post($post->post_parent);
              while (!$children && $cpost->post_parent) {
                $cpost = get_post($cpost->post_parent);
                $children = wp_list_pages("&title_li=&child_of=$cpost->ID&echo=0");
              }
            }
                    if ($children) { ?>
                    <ul id="submenu">
            ...
            <?php // added subpage navigation for home, 10 is the id of the About Us page
            
            if(is_home() ) {
            $childrenHome = wp_list_pages("&title_li=&child_of=10&echo=0");
                    if ($childrenHome) { ?>
                    <ul id="submenu">
                    <?php echo $childrenHome; ?>
                    </ul>
...
/*
if (is_page() && $post->post_parent ) {
    $children2 = wp_list_pages("&title_li=&child_of=".$post->post_parent."&echo=0");
	echo '<h1>'.$post->post_parent.'</h1>';
	// echo '<h1>'.$children2.'</h1>';
	// echo '<h1>This is a subpage</h1>';
} else {
	echo '<h1>This is NOT a subpage</h1>';
}
