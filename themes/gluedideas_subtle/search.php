...
<?php elseif (is_month()) : ?>
				<h2><?php _e('Archives for', 'gluedideas_subtle') ?> <?php the_time('F, Y'); ?></h2>
<?php elseif (is_category()) : ?>
				<h2><?php single_cat_title(''); ?> <?php _e('Archive', 'gluedideas_subtle') ?></h2>
<?php else : ?>
...
<?php while (have_posts()) : the_post(); ?>
				...
				<div id="post_<?php the_ID(); ?>" class="post summary">
...
<?php if (is_author()) : ?>
			<div id="author" class="prominent reduced"><div class="inner">
				...
				<ul>
					...
					<li class="icon feed"><a href="<?php get_author_rss_link(true, $authordata->ID, $authordata->user_nicename); ?>"><?php _e("Author's Feed", 'gluedideas_subtle'); ?></a></li>
				</ul>
			</div></div>
<?php else : ?>
